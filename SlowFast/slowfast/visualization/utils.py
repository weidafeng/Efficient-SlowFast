...
def get_confusion_matrix(preds, labels, num_classes, normalize="true"):
    ...
    # print(labels)
    # print(preds)
    # wdf add, labels 是单个张量，维度为[],无法cat，所以先做一步变换
    # labels = [torch.tensor([x]) for x in labels]
    # preds = [torch.tensor([torch.argmax(p)]) for p in preds]
...
def plot_confusion_matrix(cmtx, num_classes, class_names=None, figsize=(64, 48)):
    """
