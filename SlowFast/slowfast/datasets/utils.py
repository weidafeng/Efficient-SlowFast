...
def pack_pathway_output(cfg, frames):
    ...
    if cfg.MODEL.ARCH in cfg.MODEL.SINGLE_PATHWAY_ARCH:
        ...
        # 1. index_select(input, dim, index, out=None) -> Tensor
        # 2. linspace(start, end, steps=100) -> Tensor. The output tensor is 1-D of size :attr:`steps`.
        num_slow_frames = frames.shape[1] // cfg.SLOWFAST.ALPHA
        slow_pathway = torch.index_select(
            ...
            torch.linspace(
                0,
                frames.shape[1] - 1,
                frames.shape[1] // cfg.SLOWFAST.ALPHA
            ).long(),
        )
...
def pack_pathway_output_in_the_middle(cfg, frames):
    """
    Prepare output as a list of tensors. Each tensor corresponding to a
    unique pathway.
    Args:
    ...
    if cfg.MODEL.ARCH in cfg.MODEL.SINGLE_PATHWAY_ARCH:
        ...
        slow_pathway = torch.index_select(
            ...
            1,
            torch.linspace(
                2, frames.shape[1] - 3, frames.shape[1] // cfg.SLOWFAST.ALPHA
            ).long(),
        )
        frame_list = [slow_pathway, fast_pathway]
    else:
