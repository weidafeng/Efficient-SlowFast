...
class VideoReader(object):
    def __init__(self, cfg):
        ...
        if cfg.DEMO.OUTPUT_FILE != "":
            self.output_file = self.get_output_file(cfg.DEMO.OUTPUT_FILE, FPS=30)  # WDF ADD
    ...
    def __next__(self):
        ...
        if not was_read:
            ...
            # self.cap.set(4, self.display_height)  # 设置摄像头分辨率，3为高，4为宽
            frame = None
...
def demo(cfg):
    ...
    seq_len = cfg.DATA.NUM_FRAMES * cfg.DATA.SAMPLING_RATE  # will be downsampled
    print(cfg.DATA.NUM_FRAMES , cfg.SLOWFAST.ALPHA)
    ture_seq_len = cfg.DATA.NUM_FRAMES + (cfg.DATA.NUM_FRAMES // cfg.SLOWFAST.ALPHA)
    frames = []
    pred_labels = []
    s = 0.0
    for able_to_read, frame in frame_provider:
        ...
        if len(frames) == seq_len:
            ...
            # only considering the forward processing time
            true_fps_start = time()
            ...
            if cfg.DETECTION.ENABLE:
                ...
                preds = model(inputs)
            true_fps_end = time()
            print('[INFO] True fps (only considering the forward processing): {}'.format(
                ture_seq_len / (true_fps_end - true_fps_start)
            ))
            print('Sequence length:{}, and Time:{}'.format(ture_seq_len, true_fps_end - true_fps_start))
        ...
        if s != 0:
            fps = 1 * ture_seq_len / (s)
            print("Speed: {:.2f}FPS".format(fps))

            # Display prediction speed.
            cv2.putText(
                frame,
                "Speed: {:.2f}FPS".format(fps),
                (10, 25),
                fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                fontScale=0.65,
                color=(0, 235, 0),
